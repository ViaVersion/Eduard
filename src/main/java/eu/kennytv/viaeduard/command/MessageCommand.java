...
public final class MessageCommand extends Command {
    ...
    public void action(final String[] args, final MessageReceivedEvent event) {
        if (args.length < 2) {
            help(event.getChannel(), event.getMessage());
            return;
        }
        ...
        if (mentionedChannels.isEmpty() || mentionedChannels.get(0).getType() != ChannelType.TEXT) {
            error(event.getChannel(), event.getMessage(), "Invalid channel");
            return;
        }

        final TextChannel channel = mentionedChannels.get(0);
        EmbedMessageUtil.sendMessage(event.getChannel(), "Sent the message to " + channel.getAsMention() + " :)");
